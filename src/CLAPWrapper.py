...
class CLAPWrapper():
    ...
    def __init__(self, model_fp, use_cuda=False):
        ...
        config_file_path = Path('./src/configs/config.yml')
        self.config_as_str = config_file_path.read_text()
    ...
    def load_clap(self):
        ...
        model_state_dict = torch.load(
            self.model_fp, map_location=torch.device('cpu'))['model']
    ...
    def default_collate(self, batch):
        ...
        if isinstance(elem, torch.Tensor):
            ...
            if elem_type.__name__ == 'ndarray' or elem_type.__name__ == 'memmap':
                ...
                if self.np_str_obj_array_pattern.search(
            ...
            return {
                key: self.default_collate([d[key] for d in batch])
                for key in elem
            }
        elif isinstance(elem, tuple) and hasattr(elem,
                                                 '_fields'):  # namedtuple
            return elem_type(*(self.default_collate(samples)
    ...
    def load_audio_into_tensor(self,
                               audio_path,
                               audio_duration,
                               resample=False):
        ...
        if audio_duration * sample_rate >= audio_time_series.shape[0]:
            repeat_factor = int(
                np.ceil((audio_duration * sample_rate) /
                        audio_time_series.shape[0]))
            # Repeat audio_time_series by repeat_factor to match audio_duration
            audio_time_series = audio_time_series.repeat(repeat_factor)
            # remove excess part of audio_time_series
            audio_time_series = audio_time_series[0:audio_duration *
                                                  sample_rate]
        else:
            # audio_time_series is longer than predefined audio duration,
            # so audio_time_series is trimmed
            start_index = random.randrange(audio_time_series.shape[0] -
    ...
    def preprocess_audio(self, audio_files, resample):
        ...
        for audio_file in audio_files:
            audio_tensor = self.load_audio_into_tensor(audio_file,
                                                       self.args.duration,
                                                       resample)
            audio_tensor = audio_tensor.reshape(
                1, -1).cuda() if self.use_cuda and torch.cuda.is_available(
    ...
    def preprocess_text(self, text_queries):
        ...
        for ttext in text_queries:
            tok = self.tokenizer.encode_plus(text=ttext,
                                             add_special_tokens=True,
                                             max_length=self.args.text_len,
                                             pad_to_max_length=True,
                                             return_tensors="pt")
            for key in self.token_keys:
                tok[key] = tok[key].reshape(
                    -1).cuda() if self.use_cuda and torch.cuda.is_available(
    ...
    def get_audio_embeddings_per_batch(self, audio_files, batch_size):
        ...
        return self._generic_batch_inference(self.get_audio_embeddings,
                                             audio_files, batch_size)
    ...
    def get_text_embeddings_per_batch(self, class_labels, batch_size):
        ...
        return self._generic_batch_inference(self.get_text_embeddings,
                                             class_labels, batch_size)

    def classify_audio_files_per_batch(self, audio_files, class_labels,
                                       batch_size):
        r"""Compute classification probabilities for each audio recording in a batch and each class label"""
        return self._generic_batch_inference(self.classify_audio_files,
                                             audio_files, class_labels,
                                             batch_size)
